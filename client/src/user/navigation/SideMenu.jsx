    ...
    return (
        ...
        >
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                                ...
                                >
                                    <path
                                        ...
                                        d="M3 9l3 3l-3 3"
                                    >
                                        <animate fill="freeze" attributeName="strokeDashoffset" dur="0.2s" values="10;0" />
                                    </path>
                                    <path strokeDasharray="16" strokeDashoffset="16" d="M5 5h14">
                                        <animate fill="freeze" attributeName="strokeDashoffset" begin="0.2s" dur="0.2s" values="16;0" />
                                    </path>
                                    <path strokeDasharray="10" strokeDashoffset="10" d="M10 12h9">
                                        <animate fill="freeze" attributeName="strokeDashoffset" begin="0.4s" dur="0.2s" values="10;0" />
                                    </path>
                                    <path strokeDasharray="16" strokeDashoffset="16" d="M5 19h14">
                                        <animate fill="freeze" attributeName="strokeDashoffset" begin="0.6s" dur="0.2s" values="16;0" />
                                    </path>
                                </g>
                            </svg>
                        </span>
            ...
            <div className="p-2 pt-0 text-background dark:text-light">
                {!collapsed && user && (
                    <div>
                        <div className="px-2 mb-1 rounded-md py-1 cursor-default border-1 dark:border-background/10 bg-background/10 dark:bg-light/10 hover:border-background/20 dark:hover:border-light/20">
                            <span className="capitalize">{user.gender}&nbsp;</span>
                            <span>{user.age} year</span>
                        </div>
                        <div className="flex justify-between px-2 mb-1 rounded-md py-1 cursor-default border-1 dark:border-background/10 bg-background/10 dark:bg-light/10 hover:border-background/20 dark:hover:border-light/20">
                            <p className="flex flex-col justify-center w-full text-xl font-semibold">
                                Height
                                <span className="text-sm font-normal">{user.height} cm</span>
                            </p>
                            <p className="flex flex-col py-1 px-4 justify-center w-full text-xl font-semibold">
                                Weight
                                <span className="text-sm font-normal">{user.weight} kg</span>
                            </p>
                        </div>
                    </div>
                )}
                ...
                {RoutesData.map((menu, index) => {
                    ...
                    return (
                        <li key={index} className="group relative">
                            <Link
                                to={menu.path}
                                className={`flex items-center p-2 m-2 rounded-lg hover:bg-gray-200 text-background hover:text-background hover:dark:text-background dark:text-light transition duration-200 ease-in-out ${menu.children ? "" : isActive ? "bg-gray-300 dark:bg-gray-600 text-background" : ""}`}
                            ...
                            >
                                ...
                                {menu.children && (
                                    <span className={`${isExpanded ? "rotate-180" : ""} ${collapsed ? "hidden" : "ml-auto transition-transform duration-200"}`}>
